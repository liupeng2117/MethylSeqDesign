...
#'
#' @examples
Estimate.EDR.from.pilot <- function(p.values, model, N0, target.N, thresh.p = 0.005, FDR = 0.05, M = 10, method = "CBUM+CDD") {
    
    Fitted.Model = switch(method, `CBUM+CDD` = MixtureModel.Fittting.pi02(p.values, thresh.p = thresh.p, restrict = F))
    ...
    if (Fitted.Model[1] == "error") {
        ...
        if (method == "TwoBeta") {
            ...
            posterior = (lambda * dbeta(p.values.mod, r2, s2))/(dbeta(p.values.mod, r, s) * (1 - lambda) + lambda * dbeta(p.values.mod, 
                r2, s2))  #prob be non-DE
        ...
        if (lambda > 0.99 | lambda < 0.5) {
            ...
            Resampling <- function(target.N) {
                ...
                transform.p.values.norm <- function(p.values.each, DE_status_each, parameter) {
                  n.old = parameter[[1]]
                  n.new = parameter[[2]]
                  
                  if (DE_status_each) {
                    # beta.estimate = model[1:2] delta = model[3]
                ...
                p.values.star.posterior = sapply(1:ngenes, function(x) transform.p.values.norm(p.values.mod[x], DE_status_posterior[x], 
                  parameter = parameter))
                ...
                Estimate_Posterior <- function(p.values.star.star) {
                  ...
                  # if(mean(p.values)>0.5+1.96*sqrt(1/(12*length(p.values)))){EDR_post=D/length(p.values)} else{EDR_post = D/(B+D)}
                  EDR_post = D/(B + D)
                  ...
                  return(c(TP_hat_post, TN_hat_post, EDR_post, Declare_post, A = A, B = B, C = C, D = D, p.values.cut = p.values.cut, 
                    FDR = FDR.unique[max(index)]))
                }
                
                Estimate.Posterior.Result = matrix(apply(p.values.star.posterior, 2, Estimate_Posterior), ncol = length(target.N))
                round(Estimate.Posterior.Result, 3)
            ...
            pred <- list(EDR = ave.result["EDR", ], DeclareDMR = ave.result["DeclareDMR", ], FDR = ave.result["FDR", ])
            return(pred)
        }
    }
}
...
MixtureModel.Fittting.pi02 <- function(p.values, restrict = TRUE, l.upper = 0.95, thresh.p = 0.05) {
    ...
    if (unique(tryCatch(print(c(optim(init, fBUM, d = p.values, method = "L-BFGS-B", upper = c(r.upper, s.upper), lower = c(r.lower, 
        s.lower))$par)), error = function(e) {
        print("error")
    }) == "error")) {
        return(init)
    } else {
